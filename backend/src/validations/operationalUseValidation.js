...
const operationalUseSchema = z.object({
    productId: z.string().uuid('Produto invÃ¡lido'),
    locationId: z.string().uuid('Local de estoque invÃ¡lido'),
    ...
    if (data.periodFrom > data.periodTo) {
        context.addIssue({
            ...
            path: ['periodTo'],
            message: 'O fim do perÃ­odo deve ser igual ou posterior ao inÃ­cio'
        });
    }
});
