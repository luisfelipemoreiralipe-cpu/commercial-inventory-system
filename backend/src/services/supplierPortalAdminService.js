...
const {normalizeEmail}=require('./supplierPortalAuthService');
const orgScope=async establishmentId=>{const est=await prisma.establishments.findUnique({where:{id:establishmentId},select:{organizationId:true}});if(!est?.organizationId)throw new AppError('Organização não encontrada.',404);return est.organizationId;};
const ensureSupplier=async(id,organizationId)=>{const item=await prisma.organizationSupplier.findFirst({where:{id,organizationId}});if(!item)throw new AppError('Fornecedor central não encontrado.',404);return item;};
const list=async(supplierId,establishmentId)=>{const organizationId=await orgScope(establishmentId);await ensureSupplier(supplierId,organizationId);return prisma.supplierPortalUser.findMany({where:{organizationSupplierId:supplierId},select:{id:true,name:true,email:true,isActive:true,isBlocked:true,failedLoginAttempts:true,lockedUntil:true,lastLoginAt:true,revokedAt:true,createdAt:true},orderBy:{name:'asc'}});};
